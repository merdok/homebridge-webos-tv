...
const EVENT_EMITTER_SETUP_FINISHED = 'tvSetupFinished';
const EVENT_EMITTER_PIXEL_REFRESHER_STARTED = 'tvPixelRefresherStarted';
const EVENT_EMITTER_TV_TURNED_OFF = 'tvTurnedOff';
const EVENT_EMITTER_TV_TURNED_ON = 'tvTurnedOn';
const EVENT_EMITTER_SCREEN_SAVER_TURNED_ON = 'screenSaverTurnedOn';
const EVENT_EMITTER_POWER_STATE_CHANGED = 'powerStateChanged';
const EVENT_EMITTER_AUDIO_STATE_CHANGED = 'audioStatusChanged';
const EVENT_EMITTER_FOREGROUND_APP_CHANGED = 'foregroundAppChanged';
const EVENT_EMITTER_LIVE_TV_CHANNEL_CHANGED = 'liveTvChannelChanged';
const EVENT_EMITTER_SOUND_OUTPUT_CHANGED = 'soundOutputChanged';
const EVENT_EMITTER_NEW_APP_ADDED = 'appAdded';
const EVENT_EMITTER_APP_REMOVED = 'appRemoved';
const EVENT_EMITTER_VOLUME_UP = 'volumeUp'; // not directly used but the event is present
const EVENT_EMITTER_VOLUME_DOWN = 'volumeDown'; // not directly used but the event is present
...
class LgTvController extends EventEmitter {
  ...
  prepareTvVariables() {
    ...
    this.screenOn = true; // internal property to keep track if the screen is turned off or on, is there an event from the tv which can tell me that instead?
  }
  ...
  setupTvConnection() {
    this.getTvInformation().then(() => {
      ...
      this.logInfo(`TV webOS version: ${this.extractWebOsVersion()}`);
      ...
      this.subscribeToServices().then(() => {
        ...
        this.connectToPointerInputSocket().then(() => {
          ...
          this.emit(EVENT_EMITTER_SETUP_FINISHED);
        });
      });
    });
  }
  ...
  async subscribeToPowerStatusChange() {
    return this.tvSubscribe(WEBOS_URI_POWER_STATE, null, (err, res) => {
      ...
      // if pixel refresher started then notify listeners, only when status processing is empty to prevent multiple calls
      if (this.isPixelRefresherRunning() === true && !statusProcessing) {
        this.screenOn = false;
        this.logInfo(`Pixel refresher started!`);
        this.emit(EVENT_EMITTER_PIXEL_REFRESHER_STARTED, res);
      }

      // if tv is off then disconnect from TV
      if (this.checkTvState() === 'Off' && statusProcessing === 'Request Suspend') {
        this.screenOn = false;
        this.logInfo(`TV turned off!`);
        this.emit(EVENT_EMITTER_TV_TURNED_OFF, res);
      }

      // notify listeners that the tv was turned on
      if (this.isTvOn() === true && !statusProcessing) {
        this.screenOn = true;
        this.logInfo(`TV turned on!`);
        this.emit(EVENT_EMITTER_TV_TURNED_ON, res);
      }

      // if screen saver started then notify listeners
      if (this.checkTvState() === 'Screen Saver') {
        this.logInfo(`Screen saver started!`);
        this.emit(EVENT_EMITTER_SCREEN_SAVER_TURNED_ON, res);
      }

      this.emit(EVENT_EMITTER_POWER_STATE_CHANGED, res);
    }).catch((error) => {
  ...
  async subscribeToAudioStatusChange() {
    return this.tvSubscribe(WEBOS_URI_AUDIO_STATUS, null, (err, res) => {
      ...
      this.emit(EVENT_EMITTER_AUDIO_STATE_CHANGED, res);
    });
  }
  ...
  async subscribeToForegroundAppChange() {
    return this.tvSubscribe(WEBOS_URI_FOREGROUND_APP_INFO, null, (err, res) => {
      if (res && res.appId && res.appId.length > 0) {
        ...
        if (this.foregroundApp.appId === WEBOS_LIVE_TV_APP_ID && this.currentLiveTvChannel === false) {
          this.subscribeToChannelChange()
        }

        this.emit(EVENT_EMITTER_FOREGROUND_APP_CHANGED, res);
      }
    });
  }
  ...
  async subscribeToChannelChange() {
    return this.tvSubscribe(WEBOS_URI_CURRENT_CHANNEL, null, (err, res) => {
      if (!res || err || res.errorCode) {
        this.currentLiveTvChannel = false; // false means not subscribed to channel listener, because when trying to subscribe live tv was not active
      } else {
        if (this.getCurrentLiveTvChannelId() !== res.channelId) {
          ...
          this.emit(EVENT_EMITTER_LIVE_TV_CHANNEL_CHANGED, res);
        }
      }
    });
  }
  ...
  async subscribeToSoundOutputChange() {
    return this.tvSubscribe(WEBOS_URI_SOUND_OUPUT, null, (err, res) => {
      if (this.activeSoundOutput !== res.soundOutput) {
        ...
        this.emit(EVENT_EMITTER_SOUND_OUTPUT_CHANGED, res);
      }
    });
  }
    ...
    return this.tvSubscribe(WEBOS_URI_LAUNCH_POINTS, null, (err, res) => {
      if (res && res.launchPoints && Array.isArray(res.launchPoints)) {
        ...
        if (res.change === 'added') {
          ...
          this.emit(EVENT_EMITTER_NEW_APP_ADDED, inputInfo);
        }

        if (res.change === 'removed') {
          this.logDebug(`App removed from TV! app: ${res.title} appId: ${res.id}`);
          this.emit(EVENT_EMITTER_APP_REMOVED, inputInfo);
        }
    ...
    await new Promise((resolve, reject) => {
      this.lgtv.getSocket(WEBOS_URI_REMOTE_POINTER_SOCKET_INPUT, (err, sock) => {
        if (!err) {
          ...
          this.logDebug(`Remote control socket error - ${error}`);
        }
        resolve();
      });
    });
  ...
  isTvOn() {
    if (this.connected === true) {
      if (this.checkTvState() === 'On' || this.isScreenSaverActive()) {
        return true;
      }
    }
    return false;
  ...
  isPixelRefresherRunning() {
    if (this.checkTvState() === 'Pixel Refresher') {
      return true;
    }
    return false;
  }
  ...
  getForegroundAppAppId() {
    if (this.foregroundApp && this.foregroundApp.appId) {
          ...
          this.lgtv.request(methodUri, payload, (err, res) => {
            if (!res || err || res.errorCode || res.errorText || !res.returnValue) {
              if (res && Object.keys(res).length > 0) {
                this.logDebug(`Request failed. Error text: ${ res.errorText}, Error code: ${ res.errorCode}`);
              } else {
                this.logDebug(`Unknown error while requesting ${method} from service ${service}`);
              }
            } else {
        ...
        return new Promise((resolve, reject) => {
          ...
          this.lgtv.subscribe(methodUri, payload, (err, res) => {
            if (!res || Object.keys(res).length == 0 || err || res.errorCode || res.errorText) {
              if (res && Object.keys(res).length > 0) {
                this.logDebug(`Could not subscribe. Error text: ${ res.errorText}, Error code: ${ res.errorCode}`);
              } else {
                this.logDebug(`Unknown error while subscribing to ${method} from service ${service}`);
              }
              reject(`Error while subscribing to ${method} from service ${service}`);
              return;
            } else {
              this.logDeepDebugTvResponse(res); // log the Tv response
              delete res['returnValue']; // we do not need the return value
  ...
  async turnOn() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(EVENT_EMITTER_TV_TURNED_ON, resolve)
    })
  }
  ...
  async turnOff() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(EVENT_EMITTER_TV_TURNED_OFF, resolve)
    })
  }
  ...
  turnOffTvScreen() {
    ...
    this.tvRequest(WEBOS_URI_TURN_OFF_SCREEN, payload, ``).then(() => {
      this.screenOn = false;
    });
  }
  ...
  turnOnTvScreen() {
    ...
    this.tvRequest(WEBOS_URI_TURN_ON_SCREEN, payload, ``).then(() => {
      this.screenOn = true;
    });
  }
  ...
  async openLiveTv() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(EVENT_EMITTER_FOREGROUND_APP_CHANGED, resolve)
    })
  }
  ...
  checkTvState() {
    ...
    if (this.powerState.processing && this.powerState.processing.includes('Suspend')) {
      return 'Off';
    }

    if (this.powerState.state && this.powerState.state === 'Active Standby') {
    ...
    if (this.powerState.state && this.powerState.state === 'Active') {
      return 'On';
    }
  ...
  webOs2xTvTurnedOn() {
    // fake tv state
    this.powerState = {};
    this.powerState.state = 'Active';
    // inform listeners
    this.screenOn = true;
    this.logInfo(`TV turned on!`);
    this.emit(EVENT_EMITTER_TV_TURNED_ON, {});
  }
  ...
  webOs2xTvTurnedOff() {
    ...
    this.screenOn = false;
    this.logInfo(`TV turned off!`);
    this.emit(EVENT_EMITTER_TV_TURNED_OFF, {});
  }
